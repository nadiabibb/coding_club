...
custom_colors<-c('plum3','deeppink3','mediumorchid3','slateblue1','lightsteelblue','royalblue1')
#Need to add rent
...
ggplot(Spending_summary, aes(x = "", y = Total_Amount, fill = Type, label = scales::dollar(Total_Amount))) +
  geom_bar(stat = "identity") +
  geom_text(aes(x = 1.2, label = scales::dollar(Total_Amount)), position = position_stack(vjust = 0.5)) +
  coord_polar("y", start = 0) +
  theme_void() +
  labs(title = "Expense Breakdown by Type")+theme(plot.title = element_text(hjust = 0.5))
...
dwcolors<-c(Deposit='chartreuse3',Withdrawal='cornflowerblue')
df<-df %>%
